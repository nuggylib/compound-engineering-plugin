...
Check each agent's returned JSON against the **merge-tier** field list (compact return format). Do not require detail-tier fields (`evidence`) in the compact return -- evidence is loaded from artifact files in step 3.3.
- Drop findings missing any merge-tier required field (title, severity, section, why_it_matters, finding_type, autofix_class, confidence)
- Drop findings with invalid enum values
- Note the agent name for any malformed output in the Coverage section

### 3.2 Confidence Gate

Suppress findings below 0.50 confidence. Store them as residual concerns for potential promotion in step 3.5.

### 3.3 Batch-Load Evidence from Artifacts

Before dedup, load evidence from all per-agent artifact files. This evidence serves both dedup (evidence union) and headless presentation.

For each reviewer in the findings set, read the artifact file at `.context/compound-engineering/document-review/{run_id}/{reviewer_name}.json`. Build an evidence lookup indexed by reviewer name and finding fingerprint (`normalize(section) + normalize(title)`).

**R9 fallback detection:** If an artifact file is missing for a reviewer, check whether that reviewer's compact return contains `evidence` arrays in its findings (per-finding check). If the `evidence` key is present and non-empty in a finding, use it directly (the agent fell back to full inline returns). If absent, proceed without evidence for that finding -- graceful degradation.
...
- If the findings recommend **opposing actions** (e.g., one says cut, the other says keep), do not merge -- preserve both for contradiction resolution in 3.6
- Otherwise merge: keep the highest severity, keep the highest confidence, union evidence from the batch-loaded artifacts using reviewer name + fingerprint to locate each finding's evidence array, note all agreeing reviewers (e.g., "coherence, feasibility"). If evidence is already inline (R9 fallback), use it directly without artifact lookup
...
**Headless mode:** Do not use interactive question tools. Output all non-auto findings as a structured text summary the caller can parse and act on. Include evidence from the batch-loaded artifacts for each finding.
...
If evidence is unavailable for a finding (artifact missing and no inline fallback), omit the Evidence lines for that finding.
