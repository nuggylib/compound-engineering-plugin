...
Ask up to 3 questions targeting the unknowns that would most change the plan. Use the platform question tool when available (AskUserQuestion / request_user_input / ask_user). Fallback: present numbered options and wait for a reply.
...
After structuring the plan, ask the user how they want to receive it using the platform question tool (AskUserQuestion / request_user_input / ask_user). Fallback: present numbered options and wait for a reply.
